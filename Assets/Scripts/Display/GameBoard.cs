...
public class GameBoard : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        board  = Utility.FlipHorizontaly2DArray( gameLogic.GetComponent<GameLogic>().gameState.board);
        if (player == 1)
        {
    ...
    public void hightlightLastMove(string move)
    {
        currentMove = gameLogic.GetComponent<GameLogic>().gameState.currentPlayer == 0? 1:0;
        from = ChessGameLogic.stringPositionToBoard(move.Substring(0,2));
        to = ChessGameLogic.stringPositionToBoard(move.Substring(2,2));
        UpdateBoard();
        UpdatePices();
    }
    private void SetUp() 
    {
        currentMove =  gameLogic.GetComponent<GameLogic>().gameState.currentPlayer == 1 ? 0 : 1; 
